...
export const api = {
  ...
  listInteractiveFiles(id: string): Promise<{ tree: FileNode; cwd?: string }> {
    return request<{ tree: FileNode; cwd?: string }>(`/interactive/sessions/${id}/files`)
  },
  // Unggah berkas ke folder MANA PUN di project sesi (tidak menimpa project).
  async importInteractiveChunk(
    id: string,
    path: string,
    first: boolean,
    blob: Blob,
    ...
    const q = new URLSearchParams({ path, first: first ? '1' : '0' })
    const res = await fetch(
      `${API_PREFIX}/interactive/sessions/${id}/import/chunk?${q.toString()}`,
      { method: 'POST', headers, body: blob },
    )
    if (res.status === 401) {
      clearToken()
