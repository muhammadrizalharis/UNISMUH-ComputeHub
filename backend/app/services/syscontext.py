...
async def system_context(python_version: str | None) -> str:
    ...
    async with _lock:
        ...
        text = (
            ...
            "- DIREKTORI KERJA notebook = /persist (folder yang sama dengan menu "
            "Penyimpanan), kecuali sesi memuat project (ZIP/folder/GitHub) -> /work. "
            "Jadi path RELATIF apa adanya seperti yang terlihat di menu Penyimpanan "
            "(mis. `data/berkas.csv`) LANGSUNG bekerja; tak perlu menulis prefiks "
            "/persist (boleh, tapi tidak wajib).\n"
